...
def pick(rows, max_picks, skip=()):
    ...
    picks = [r for r in ranked if r["score"] is not None and r["marketplace"] not in skipped]
...
def read_state(value):
    """Validate the previous run's state block. Returns None when it cannot be trusted."""
    try:
        ...
        return {"version": 1, "rows": clean}
    except (AttributeError, ValueError):
        return None


def next_state(previous, ranked, picks):
    ...
    for row in ranked:
        ...
        elif marketplace in picked and prior["picked_runs"] > 0:
    ...
    return {"version": 1, "rows": rows}, changes
```
