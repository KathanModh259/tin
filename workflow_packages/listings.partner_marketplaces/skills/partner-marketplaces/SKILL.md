...
2. Read `### Feature map` when present, and any positioning or competitor notes the index
   ...
   instructions.
3. Resolve the product website: the `product_url` input, else the site named in memory. If
   neither exists, gates that need a public page are `unknown`.
4. Find the newest report under `reports/listings/` and parse its `tin-listings-state` JSON
   block with `read_state()`. If it returns None, say continuity is unavailable and carry on.
5. Map the `skip` input to marketplace ids. Words you cannot map are quoted back in the report,
   not guessed.
...
Build one row per marketplace and call `pick(rows, max_picks, skip)`. Rate `audience` from memory
and cite the line. Do not change a score by hand. If no row scores, the decision is "Nothing to
submit this week", followed by the single change that would unblock the best blocked row.
...
- Use the product's own words and any writing-style skill in project Files. No claims memory
...
Call `next_state(previous, ranked, picks)`. Write REPORT.md's layout to `context.output.path`
with Python, inserting the checked values instead of retyping them. Write nothing else.
