...
3. Verify every candidate with a search before including it:
   - Is there a currently open call for speakers or guests? Find the exact deadline (date, and
     timezone if the venue states one) on the venue's own CFP/apply page, not a secondhand
     aggregator or a past year's page. Note what you checked and when.
   ...
   Drop anything whose call appears closed, has no verifiable open deadline, or whose
   requirements you cannot confirm from the venue's own page. Do not guess a deadline.

4. Skip anything named in `already_pitched`, matching on name and allowing minor formatting
   differences.

5. Stop at `max_venues`. If more candidates survive verification, rank by nearest deadline and
   thematic fit over size or prestige of the venue.
...
7. Write the report to the declared path. Start with one line, `Status: complete` if at least
   ...
   - `Deadline:` the exact date (and timezone if stated) and its source
